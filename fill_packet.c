...
// Fill the ICMP header with type, code, ID, and sequence number
void fill_icmphdr(struct icmphdr *icmp_hdr, const char *data, int seq) {
    ...
    // Copy the payload data into the ICMP packet
    int data_len = strlen(data);
    memcpy(icmp_hdr + 1, data, data_len);

    // Calculate the checksum for the entire ICMP packet
    icmp_hdr->checksum = fill_cksum((u16 *)icmp_hdr, sizeof(struct icmphdr) + data_len);
}

// Fill the IP header with version, source/destination IP, and other attributes
void fill_iphdr(struct iphdr *ip_hdr, const char *src_ip, const char *dest_ip, int payload_len) {
    ...
    ip_hdr->ttl = 1;               // Time to Live set to 1 (for subnet scan)
